...
export const PAGE_SEO: SeoPage[] = [
  {
    ...
    bodyHtml: `<main id="route-home" class="section"><div class="container"><h1 class="display">We ship the thing you can sell.</h1><p class="lede">${BRAND_NAME} is a Palm Coast AI consultant shop. We embed working software in the operation and leave it running.</p><p>${FD_NAME} is the after-hours desk for field operators: ${FD_PRICE}. Live this week or you do not pay the setup. ${INDEXME_NAME} gets the page found before you spend more on ads.</p><p><a href="${FD_URL}">Start ${FD_NAME}</a> · <a href="${INDEXME_URL}">Open ${INDEXME_NAME}</a> · Consult ${CONSULT_DISPLAY}</p></div></main>`,
  },
  {
    path: "/about",
    ...
    bodyHtml: `<main id="route-about" class="section"><div class="container"><h1 class="display">About AgentHive Inc</h1><p class="lede">${BRAND_NAME} / ${LEGAL_NAME} is the Palm Coast shop that builds and ships. Daniel Graham embeds working AI in real operations, then leaves it running.</p><p>Contact: <a href="mailto:${CONTACT_EMAIL}">${CONTACT_EMAIL}</a>. ${FD_NAME} consult ${CONSULT_DISPLAY}. Shop address ${ADDRESS_LINE}.</p><p>This page is the company record for ${BRAND_NAME} at agenthiveinc.com — not QpiAI, not an insurance hive, not an OSS org with a similar name.</p></div></main>`,
  },
  {
    path: "/buzz",
