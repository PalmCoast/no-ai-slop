...
export const PAGE_SEO: SeoPage[] = [
  {
    ...
    description: `${BRAND_NAME} (${LEGAL_NAME}) is a Palm Coast AI consultant shop. ${FD_NAME} is the after-hours desk: ${FD_PRICE}. Grok Bots run the hive.`,
    h1: "THE HIVE. We SWARM.",
    bodyHtml: `<main id="route-home" class="section"><div class="container"><h1 class="display">THE HIVE. We SWARM.</h1><p class="lede">${BRAND_NAME} is a Palm Coast AI consultant shop. Specialized Grok Bots take a goal in plain language and run it across your tools.</p><p>${FD_NAME} is the after-hours desk for field operators: ${FD_PRICE}. Live this week or you do not pay the setup. Consult is how you buy time in the room.</p><p><a href="${FD_URL}">Start ${FD_NAME}</a> · <a href="${HIVE_CONSULT_URL}">Book the free 30</a> · <a href="/buzz">Read The Buzz</a></p></div></main>`,
  },
  {
    path: "/about",
    title: "About AgentHive Inc — AGENTHIVEINCCOM LLC, Palm Coast",
    description: `${BRAND_NAME} is the Palm Coast AI consultant shop behind ${FD_NAME}. Legal name ${LEGAL_NAME}. Contact ${CONTACT_EMAIL} or ${CONSULT_DISPLAY}.`,
    h1: "About AgentHive Inc",
    bodyHtml: `<main id="route-about" class="section"><div class="container"><h1 class="display">About AgentHive Inc</h1><p class="lede">${BRAND_NAME} / ${LEGAL_NAME} is the Palm Coast shop that builds and ships. Daniel Graham embeds working AI in real operations, then leaves it running.</p><p>Contact: <a href="mailto:${CONTACT_EMAIL}">${CONTACT_EMAIL}</a>. ${FD_NAME} voice ${CONSULT_DISPLAY}. Shop address ${ADDRESS_LINE}.</p><p>This page is the company record for ${BRAND_NAME} at agenthiveinc.com — not the homepage swarm, not The Buzz, not Rankings.</p></div></main>`,
  },
  {
    path: "/buzz",
...
export function netlifyRouteRewrites(): string[] {
  const lines: string[] = [];
  for (const page of PAGE_SEO) {
    if (page.path === "/") continue;
    const dest = `${page.path}/index.html`;
    lines.push(`${page.path} ${dest} 200!`);
    lines.push(`${page.path}/ ${dest} 200!`);
  }
