...
**Assigned to Reed.** Shop check launch notes are in [`CHECK-LAUNCH.md`](CHECK-LAUNCH.md). AskYard is already live on Netlify site `askyard-firstdeploy` at **askyard.firstdeploy.ai**. Daniel wants Marquee loud on that same site. Do not redesign AskYard. Do not rewrite the public copy in `MARKETING.md`. Do not touch Stateside, Sonaris, or the Netlify site that currently serves **firstdeploy.ai** or **agenthiveinc.com**. Do **not** create a second Netlify site. After this PR merges, attach **marquee.firstdeploy.ai** as a domain alias on the **same** AskYard site, then run the distribution checklist until people are actually asking and founders are bidding for the lights.

Public mouth: [`MARKETING.md`](MARKETING.md). Job ticket: [`REED-ASSIGNMENT.md`](REED-ASSIGNMENT.md). This file stays off camera. Never post Stripe, webhooks, demo bids, functions, Gateway, or how the meter is built. The launch checklist is **internal only**. `/launch` redirects home. Do not resurrect a public playbook page.
...
- Live site: `askyard-firstdeploy` (`7887a6d0-f351-4633-a79b-f83c6e63cf78`) at `askyard.firstdeploy.ai`
- Code: `askyard/` (Vite + React SPA + Netlify Functions + `marquee-host` edge fn)
- Brand: `askyard/BRAND.md`
- Human launch notes: `askyard/LAUNCH.md` (internal)
...
- `/launch` 301 to home (playbook stays in this file and `LAUNCH.md`)
...
1. **AskYard already has its own Netlify site** (`askyard-firstdeploy`). Use it. Do not change the site that already serves `firstdeploy.ai`. Do not change `agenthiveinc.com`, Stateside, or Sonaris. If you set the First Deploy AI site’s base directory to `askyard`, you will take firstdeploy.ai offline.
...
### 2. Stay on the existing AskYard site

The AskYard site is already live. Do **not** create another Netlify site. Do **not** change firstdeploy.ai or agenthiveinc.com settings.

1. Production branch is `main`.
2. Site settings stay:
   - Base directory: `askyard`
   - Build command: `npm run build`
   - Publish directory: `dist`
   - Functions directory: `netlify/functions` (relative to `askyard/`)
3. AI Gateway stays on. Do not paste a provider key.
4. For live Marquee bids set `STRIPE_SECRET_KEY` and `STRIPE_WEBHOOK_SECRET` on this site only. Webhook URL: `https://askyard.firstdeploy.ai/api/stripe-webhook`. Never create a fixed Stripe Price. Checkout builds `price_data.unit_amount` from the bid they typed. Without the key, demo bids still move the chart.
5. Publish a production deploy after this PR merges. Scheduled `hunt-weekly` only runs on published production.

### 3. Attach marquee.firstdeploy.ai on the same site

`askyard.firstdeploy.ai` is already attached. After merge, attach **marquee.firstdeploy.ai** as a domain alias on the **same** AskYard site. The edge function `marquee-host` rewrites that host’s `/` to `/marquee`. Do not create a second Netlify site for Marquee. Do not add a 301 from firstdeploy.ai `/` to AskYard.

DNS (pick the one that matches how firstdeploy.ai is hosted):

- If firstdeploy.ai uses Netlify DNS: add hostname `marquee` as a Netlify domain alias on the AskYard site, or a CNAME `marquee` → the AskYard `*.netlify.app` host.
- If firstdeploy.ai uses external DNS: CNAME `marquee.firstdeploy.ai` → the AskYard `*.netlify.app` host. Then verify TLS on the AskYard site.
...
Ask one new question on the live hero. Confirm an answer and a priced offer come back. Confirm the ask count on `/board` moved. On `/rep` look up AskYard. On `/marquee` type a bid at or above `minNextBid` (demo if Stripe is off) and confirm the name sits at #1. After DNS, `https://marquee.firstdeploy.ai/` should show the lights.
